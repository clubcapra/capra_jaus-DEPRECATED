...
class ClearEmergency(JausMessage):
    # as5710 p40
    code = 0x7
    data_fmt = '<H'
    ...
    def execute(self):
        rospy.logdebug('ClearEmergency code %s', self.emergency_code)
        if self.emergency_code in [EmergencyCode.STOP]:
            s_status = UInt8()
            s_status.data = Status.STANDBY
            s_emergency = UInt16()
            s_emergency.data = EmergencyCode.NO_CODE
